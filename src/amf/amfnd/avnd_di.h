...
uint32_t avnd_di_pg_act_send(struct avnd_cb_tag *, const std::string&, AVSV_PG_TRACK_ACT, bool);
...
uint32_t avnd_di_reg_su_rsp_snd(struct avnd_cb_tag *cb, const std::string& su_name, uint32_t ret_code);
uint32_t avnd_di_ack_nack_msg_send(struct avnd_cb_tag *cb, uint32_t rcv_id, uint32_t view_num);
extern void avnd_di_uns32_upd_send(int class_id, int attr_id, const std::string& dn, uint32_t value);
