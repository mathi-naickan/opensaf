...
static SaAisErrorT node_ccb_completed_modify_hdlr(CcbUtilOperationData_t *opdata)
{
	...
	while ((attr_mod = opdata->param.modify.attrMods[i++]) != NULL) {
		...
		if (!strcmp(attribute->attrName, "saAmfNodeSuFailOverProb")) {
			...
			if (value > SA_TRUE) {
				LOG_ER("Invalid saAmfNodeAutoRepair '%s'", opdata->objectName.value);
				rc = SA_AIS_ERR_BAD_OPERATION;
				goto done;
			}
			...
			uint32_t value = *((SaUint32T *)attribute->attrValues[0]);
			if (value > SA_TRUE) {
				LOG_ER("Invalid saAmfNodeFailfastOnTerminationFailure '%s'",
					opdata->objectName.value);
				rc = SA_AIS_ERR_BAD_OPERATION;
				goto done;
			}
				    ...
				    SaInvocationT invocation, SaAmfAdminOperationIdT operationId)
{
	...
	case SA_AMF_ADMIN_UNLOCKED:
		...
		if (node->su_cnt_admin_oper == 0 && invocation != 0) {
			...
			node->admin_node_pend_cbk.admin_oper = 0;
		}
	...
	case SA_AMF_ADMIN_SHUTTING_DOWN:
		...
		while (su != NULL) {
			if (su->list_of_susi != AVD_SU_SI_REL_NULL) {
				...
				/* Verify that two assigned SUs hosted by the same node does
				 * not belong to the same SG. Allow for nway-active.
				 */
			     ...
			     const SaImmAdminOperationParamsT_2 **params)
{
	...
	while (su != NULL) {
		...
		if ((su->saAmfSUPresenceState == SA_AMF_PRESENCE_INSTANTIATING) ||
			...
			rc = SA_AIS_ERR_TRY_AGAIN;
			LOG_WA("'%s' presence state is '%u'", su->name.value, su->saAmfSUPresenceState);
			goto done;
		}
