...
/****************************************************************************
  ...
  Notes         : None.
******************************************************************************/
uns32 avnd_evt_tmr_qscing_cmpl_evh(struct avnd_cb_tag *cb, struct avnd_evt_tag *evt) 
{
...
/*************************   Section 1 Ends Here **************************/
	...
	/* if (!cbk_rec && comp->pxied_list.n_nodes != 0)  */
	if (!cbk_rec) {
		TRACE("Empty comp callback record comp=%s, callback type=%llx",comp->name.value,resp->inv);
		return rc;
	}
