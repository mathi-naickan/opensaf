...
bool ImmObjCreate::immobj_update_required() {
  ...
  if (class_type == AVSV_SA_AMF_SI_ASSIGNMENT) {
	...
	TRACE_1("su_name:'%s'",su_name.value);
	break;
      }	
    }
	...
	csicomp_name = reinterpret_cast<SaNameT*>(attribute->attrValues[0]);
	...
	comp_name.value[j] = '\0';
        comp_name.length = j;
	TRACE_1("comp_name:'%s'", comp_name.value);
	break;
      }	
    }
    csi =  csi_db->find(Amf::to_string(&parentName_));
...
bool ImmObjUpdate::su_get_attr_value() {	
  bool ret = true;
  AVD_SU *su = nullptr;
  su = su_db->find(Amf::to_string(&dn_));
  if (su == nullptr) {
...
bool ImmObjUpdate::siass_get_attr_value() {	
  ...
  avd_association_namet_init(&dn_, &su_name, &si_name, AVSV_SA_AMF_SI);
...
bool ImmObjUpdate::csiass_get_attr_value() {
  ...
  avd_association_namet_init(&dn_, &comp_name, &csi_name, AVSV_SA_AMF_CSI);
...
bool ImmObjUpdate::comp_get_attr_value() {
  AVD_COMP *comp = nullptr;
  comp = comp_db->find(Amf::to_string(&dn_));	
  if (comp == nullptr) {
...
bool ImmObjUpdate::si_get_attr_value() {
  TRACE("si:'%s'",dn_.value);
  AVD_SI *si = si_db->find(Amf::to_string(&dn_));
  if (si == nullptr) {
...
bool ImmObjUpdate::immobj_update_required() {
  bool ret = true;
  AVSV_AMF_CLASS_ID class_type = object_name_to_class_type(&dn_);
  if (implementer == true)
    return true;
...
bool ImmObjDelete::is_siass_exist() { 
  ...
  //dn : safSISU=safSu=SU1\,safSg=AmfDemo\,safApp=AmfDemo1,safSi=AmfDemo,safApp=AmfDemo1
  avd_association_namet_init(&dn_, &su_name, &si_name, AVSV_SA_AMF_SI);
  TRACE("si:%s",si_name.value);
  TRACE("su:%s",su_name.value);
  AVD_SU_SI_REL *susi = avd_susi_find(avd_cb, &su_name, &si_name);
...
bool ImmObjDelete::is_csiass_exist() { 
  ...
  AVD_SU_SI_REL *susi = nullptr;
  //dn:safCSIComp=safComp=AmfDemo\,safSu=SU1\,safSg=AmfDemo\,safApp=AmfDemo1,safCsi=AmfDemo,safSi=AmfDemo,safApp=AmfDemo1
  TRACE("csiass:%s",dn_.value);
  avd_association_namet_init(&dn_, &comp_name, &csi_name, AVSV_SA_AMF_CSI);
  TRACE("csi:%s",csi_name.value);
  TRACE("comp:%s",comp_name.value);
  csi =  csi_db->find(Amf::to_string(&csi_name));
  comp = comp_db->find(Amf::to_string(&comp_name));	
  if ((csi == nullptr) || (comp == nullptr)) {
    TRACE_1("comp or csi not found");
