			...
			for (const auto& iter : sg->list_of_su) {
				if (SA_AMF_READINESS_IN_SERVICE == iter->saAmfSuReadinessState) {
					...
					if (sg->equal_ranked_su == true) {
						...
						curr_su = iter;
						TRACE("no pref_su set, curr_su is %s", curr_su->name.value);
						break;
					}
				}
			}
 ...
 * 
 **************************************************************************/
uint32_t SG_NWAY::susi_success_su_oper(AVD_CL_CB *cb,
	AVD_SU *su, AVD_SU_SI_REL *susi, AVSV_SUSI_ACT act, SaAmfHAStateT state)
{
	...
	if (susi && (SA_AMF_HA_QUIESCED == state) && (AVSV_SUSI_ACT_DEL != act)) {
		...
		if (true == is_eng) {
			...
			if (NCSCC_RC_SUCCESS != rc) {
				LOG_ER("%s:%u: %s (%u)", __FILE__, __LINE__, su_oper_list.front()->name.value,
