...
//If only one single ccb has a read lock on an object, i.e. the lock is currently
//not actually shared, then that ccb may upgrade the read-lock to an exclusive lock
...
static ObjectShortCountMap sObjectReadLockCount; //Needs to be declared above CcbInfo
...
void CcbInfo::addObjReadLock(ObjectInfo* obj, std::string& objName)
{
    ...
    if(obj->mObjFlags & IMM_SHARED_READ_LOCK) {
        TRACE_7("Object '%s' already locked for safe-read by some ccb(s)", objName.c_str());
        ...
        */
        ...
        /* A non-zero mCcbId for an object indicates exclusive lock (create/delete/modify)
           if and only if the identified ccb is still *active*. A zero mCcbId for an object
           *guarantees* that it is not exclusive locked.
           This function *requires* that the invoker has done the pre-check of no interference
           of a proposed shared reader with any pre-existing exclusive op and zeroed the ccbId
           if and only if an existing ccbId was no longer identifying an active ccb.

           There is no explicit ccb-modify lock since it can be inferred by (a) the ccb-id
           of an object identifying an active CCB and (b) neither the create or modify
    ...
    if(oscm == sObjectReadLockCount.end()) {
        ...
        (oscm->second)++;
        LOG_IN("Incremented read-lock count for %s to %u", objName.c_str(), oscm->second);
    }
...
void CcbInfo::removeAllObjReadLocks()
{
    ...
    */
    ...
    for(osi = this->mSafeReadSet.begin(); osi != this->mSafeReadSet.end(); ++osi)
    {
        ...
        if(ccbIdOfObj == 0) {
            ...
            if(oscm != sObjectReadLockCount.end())
            {
                if((oscm->second > 1) || !(ccb->isReadLockedByThisCcb(object)))
                {
                    ...
                    osafassert((oscm->second == 1) && (ccb->isReadLockedByThisCcb(object)));
                    TRACE("CcbModify: Object '%s' safe-read-locked only by this ccb (%u) => "
                         "upgrade to exclusive lock is possible", objectName.c_str(), ccbId);
    ...
    *augDelete = ccb->mAugCcbParent != NULL;
    ...
    for(int doIt=0; (doIt < 2) && (err == SA_AIS_OK); ++doIt) {
        ...
        err = deleteObject(oi, reqConn, adminOwner, ccb, doIt, objNameVector,
            connVector, continuations, pbeConnPtr?(*pbeConnPtr):0, &readLockedObject);
    ...
    ObjectInfo **readLockedObject)
{
    ...
    *readLockedObject = NULL;    /* readLockedObject cannot be NULL */
    ...
    if(ccbIdOfObj != ccb->mId) {
        ...
        if(ccbIdOfObj == 0) {
            ObjectShortCountMap::iterator oscm = sObjectReadLockCount.find(oi->second);
            if(oscm != sObjectReadLockCount.end()) {
                if((oscm->second > 1) || !(ccb->isReadLockedByThisCcb(oi->second))) {
                    ...
                    osafassert((oscm->second == 1) && (ccb->isReadLockedByThisCcb(oi->second)));
                    TRACE("CcbDelete: Object '%s' safe-read-locked only by this ccb (%u) => "
                         "upgrade to exclusive lock is possible", oi->first.c_str(), ccb->mId);
    ...
    if(ccbIdOfObj != ccbId) {
       i1 = std::find_if(sCcbVector.begin(), sCcbVector.end(), 
        ...
        /* Set ccb-id to zero to signify no exclusive lock set.
           CcbId of 0 does not mean that a shared read lock must be set for the object.
           It only means that zero or more shared read locks *could* be set for this object.
         */
        obj->mCcbId = 0;
    } else {
        TRACE_7("Safe-read: Object '%s' is currently *exclusive* locked by *same* ccb %u",
        ...
        /* Read-lock succeeds as a no-op.
           ...
           requested locking for the ccb that fails, it is the safe-read-access done later.
           The safe read of the object will fail with ERR_NOT_EXIST because the object is
           scheduled for delete in the *same* ccb as the reader. If the object was locked
           for delete in a different ccb then the safe-reader in this ccb would get ERR_BUSY.
        */
        osafassert(err == SA_AIS_OK);
        goto done;
    }
...
ImmModel::objectIsLockedByCcb(const ImmsvOmSearchInit* req)
{
    ...
    if(ccbIdOfObj == ccbId) {
        ...
        i1 = std::find_if(sCcbVector.begin(), sCcbVector.end(), 
            CcbIdIs(ccbIdOfObj));
        if ((i1 != sCcbVector.end()) && ((*i1)->isActive())) {
            TRACE_7("ERR_BUSY: Safe-read: ccb id %u differs from active ccb id on object %u", 
                ccbId, ccbIdOfObj);
            err = SA_AIS_ERR_BUSY;
            goto done;
        }
    ...
    */
    ...
    if(ccbIdOfObj == 0) {
        TRACE_7("Object '%s' is currently not locked by this ccb %u", objectName.c_str(), ccbId);
        err = SA_AIS_ERR_INTERRUPT;
        goto done;
    }
...
ImmModel::accessorGet(const ImmsvOmSearchInit* req, ImmSearchOp& op)
{
    ...
    if(i->second->mObjFlags & IMM_CREATE_LOCK) {
        ...
        if(isSafeRead) {
            if(req->ccbId != i->second->mCcbId) {
                TRACE_7("ERR_NOT_EXIST: Object '%s' is being created but not in this ccb.",
                    objectName.c_str());  
                err = SA_AIS_ERR_NOT_EXIST;
                goto accessorExit;
            }
        ...
        /* Interference with unapplied create taken care of above in if-branch for
           ...
           Regular accessor-get is not affected by open delete, modify, or safe-reads.
           It simply accesses the current committed version.

           Safe-read *is* affected by unapplied modify or delete, hence this else branch.

           Mdify in same ccb => safe-read must read the after-image-version in the same ccb.
           Delete in same ccb => the object becomes inaccesible (ERR_NOT_EXIST) for safe-read.
           Mofify or delete by *other* ccb => safe-read should have been rejected (ERR_BUSY)
           in objectIsLockedByCcb().

           Safe-read is of course not affected by other safe-read operations on this object
           in same or other ccbs. Latest commited version is read. 
        */

        i1 = std::find_if(sCcbVector.begin(), sCcbVector.end(), CcbIdIs(req->ccbId));
        osafassert(i1 != sCcbVector.end()); /*Already checked in objectIsLockedByCcb. */
        ccb = *i1;
        if((i->second->mCcbId == 0) && !(ccb->isReadLockedByThisCcb(i->second))) {
            LOG_WA("Safe read on object %s NOT locked by ccb (%u). Should not be caught here",
                objectName.c_str(), req->ccbId);
            /* If we get here this may be seen as a bug to be be corrected. Should perhaps
               assert here, but be lenient for now and just report "INTERRUPT" which should
               result in an new attempt to lock the object for safe-read.
               The safe-read lock should have been obtained before reacing accessor-get.
             */
        ...
        if(i->second->mCcbId != req->ccbId) {
            if(i->second->mCcbId == 0) {
                TRACE_7("Object %s locked for shared read in ccb (%u) and possibly other ccbs",
                    objectName.c_str(), req->ccbId);
                osafassert(i->second->mObjFlags & IMM_SHARED_READ_LOCK);
                obj = i->second; /* Use latest committed version. */
            } else {
                LOG_WA("Safe read on object %s exclusive locked by other ccb (%u). "
                    "Should not be caught here", objectName.c_str(), i->second->mCcbId);
                /* If we get here this may be seen as a bug to be be corrected. Should perhaps
            ...
            TRACE_7("Safe read on object *exclusive* locked by *this* ccb %u", req->ccbId);
    ...
    //SafeRead works exactly like accessor get with respect to runtime attributes.
...
ImmModel::searchInitialize(ImmsvOmSearchInit* req, ImmSearchOp& op)
{
    ...
    if(req->ccbId) {
        LOG_WA("SafeRead ended up in ImmModel::searchInitialize ccb:%u", req->ccbId);
        err = SA_AIS_ERR_LIBRARY;
        goto searchInitializeExit;
    }
               ...
               abortCcb)  /* Request to abort ALL non critical CCBs */
            {
                ...
                if((*i3)->mState == IMM_CCB_CRITICAL) {
                    ...
                    /* oi_timeout set to 0 means that the ccb should be added to the vector.
                       CCB will be aborted only after the global abort generated here by the coord
