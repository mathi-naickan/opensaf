...
SaBoolT
immModel_pbeIsInSync(IMMND_CB *cb, SaBoolT checkCriticalCcbs)
{
...
immModel_pbeNotWritable(IMMND_CB *cb) {
	...
	return (SaBoolT) ImmModel::instance(&cb->immModel)->immNotPbeWritable(false);
}
        ...
        */
        ...
        if(isSyncClient) {
            ...
            while(ai) {
                ...
                if(info->mDying) {
                    assert(info->mReleaseOnFinalize);
                }
